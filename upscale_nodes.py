...
class MiniMaxH3ChainUpscaleHandoff:
    ...
    OUTPUT_TOOLTIPS = (
        "Prepared lightweight upscale state for the next recursive step; pixel tensors are not retained.",
    )
    FUNCTION = "prepare"
...
class MiniMaxH3ChainUpscaleAdvance:
    ...
    def INPUT_TYPES(cls):
        return {"required": {
                    "next_state": (UPSCALE_STATE_TYPE, {
                        "tooltip": "Prepared lightweight state from the preceding upscale iteration."}),
                    "loop_id": ("STRING", {
                        "tooltip": "Internal identifier for the owning upscale loop node."}),
