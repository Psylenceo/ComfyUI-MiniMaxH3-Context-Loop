...
let submissionCase=await lifecycleSubmit(stale); assert.deepEqual(submissionCase.log,[["list","run"],["claim","run","exact"],["prepare","exact"],["submit","exact"]]);assert.equal(submissionCase.result.submission.prompt_id,"exact-prompt-123");
