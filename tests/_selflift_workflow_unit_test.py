...
path = ROOT / "example_workflows/Ref2V Studio SelfLift - EXPERIMENTAL - MiniMax H3 0.6.json"
...
assert nodes[project]["widgets_values"] == [False, "none", 5]
...
assert nodes[hunt]["widgets_values"] == [0, "fixed", 1, 4, "hunt_1", "taeh3.safetensors", False, True]
for i, a in enumerate(workflow["nodes"]):
    ax, ay = a["pos"]; aw, ah = a["size"]
    for b in workflow["nodes"][i+1:]:
        bx, by = b["pos"]; bw, bh = b["size"]
