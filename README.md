...
| [`carousel-layout-redesign`](https://github.com/Psylenceo/ComfyUI-MiniMaxH3-Context-Loop/tree/carousel-layout-redesign) | Reshapes the Project Asset Carousel into a source-tree left column that nests crops/edits under their source (via the existing `parent_asset_id` lineage) and collapses `-vN` tag-version families (`@char_bob`, `@char_bob-v1`, ...) into a single stacked card at their correct tree position, with a bottom panel showing the selected asset's combined versions/edits. Also fixes the node having no horizontal resize floor, matches thumbnail sizing to the detail row, and switches Review Gate's capture-family tag suffix from a bare digit to `-vN` to stop it from misreading tags like `@vehicle-van` as a version match. | Submitted as [PR #71](https://github.com/ethanfel/ComfyUI-MiniMaxH3-Context-Loop/pull/71) |
